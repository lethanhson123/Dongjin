...
export class tuser_log_chkInfoComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.tuser_log_chkService.BaseParameter.ID = Number(this.ActiveRouter.snapshot.params.ID);
  ...
  tuser_log_chkSave() {
    ...
    this.tuser_log_chkService.SaveAsync().subscribe(
      res => {
        this.tuser_log_chkService.FormData = res as tuser_log_chk;
        this.Router.navigateByUrl(environment.tuser_log_chkInfo + this.tuser_log_chkService.FormData.TUSER_IDX);
  ...
  tuser_log_chkAdd() {
    this.Router.navigateByUrl(environment.tuser_log_chkInfo + environment.InitializationNumber);
    this.tuser_log_chkService.BaseParameter.ID = environment.InitializationNumber;
    this.tuser_log_chkSearch();
  }
}
