...
export class torder_lead_bomInfoComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.torder_lead_bomService.BaseParameter.ID = Number(this.ActiveRouter.snapshot.params.ID);
  ...
  torder_lead_bomSave() {
    ...
    this.torder_lead_bomService.SaveAsync().subscribe(
      res => {
        this.torder_lead_bomService.FormData = res as torder_lead_bom;
        this.Router.navigateByUrl(environment.torder_lead_bomInfo + this.torder_lead_bomService.FormData.LEAD_INDEX);
  ...
  torder_lead_bomAdd() {
    this.Router.navigateByUrl(environment.torder_lead_bomInfo + environment.InitializationNumber);
    this.torder_lead_bomService.BaseParameter.ID = environment.InitializationNumber;
    this.torder_lead_bomSearch();
  }
}
