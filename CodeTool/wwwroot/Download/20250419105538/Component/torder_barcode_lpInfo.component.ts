...
export class torder_barcode_lpInfoComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.torder_barcode_lpService.BaseParameter.ID = Number(this.ActiveRouter.snapshot.params.ID);
  ...
  torder_barcode_lpSave() {
    ...
    this.torder_barcode_lpService.SaveAsync().subscribe(
      res => {
        this.torder_barcode_lpService.FormData = res as torder_barcode_lp;
        this.Router.navigateByUrl(environment.torder_barcode_lpInfo + this.torder_barcode_lpService.FormData.TORDER_BARCODE_IDX);
  ...
  torder_barcode_lpAdd() {
    this.Router.navigateByUrl(environment.torder_barcode_lpInfo + environment.InitializationNumber);
    this.torder_barcode_lpService.BaseParameter.ID = environment.InitializationNumber;
    this.torder_barcode_lpSearch();
  }
}
