...
export class ttc_bomInfoComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.ttc_bomService.BaseParameter.ID = Number(this.ActiveRouter.snapshot.params.ID);
    this.ttc_bomSearch();
  }
  ttc_bomSearch() {
  ...
  ttc_bomSave() {
    ...
    this.ttc_bomService.SaveAsync().subscribe(
      res => {
        this.ttc_bomService.FormData = res as ttc_bom;
        this.Router.navigateByUrl(environment.ttc_bomInfo + this.ttc_bomService.FormData.TTC_BOM_IDX);
  ...
  ttc_bomAdd() {
    this.Router.navigateByUrl(environment.ttc_bomInfo + environment.InitializationNumber);
    this.ttc_bomService.BaseParameter.ID = environment.InitializationNumber;
    this.ttc_bomSearch();
  }
}
