...
export class tsyear_inventory_histInfoComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.tsyear_inventory_histService.BaseParameter.ID = Number(this.ActiveRouter.snapshot.params.ID);
  ...
  tsyear_inventory_histSave() {
    ...
    this.tsyear_inventory_histService.SaveAsync().subscribe(
      res => {
        this.tsyear_inventory_histService.FormData = res as tsyear_inventory_hist;
        this.Router.navigateByUrl(environment.tsyear_inventory_histInfo + this.tsyear_inventory_histService.FormData.TSYEAR_INV_IDX);
  ...
  tsyear_inventory_histAdd() {
    this.Router.navigateByUrl(environment.tsyear_inventory_histInfo + environment.InitializationNumber);
    this.tsyear_inventory_histService.BaseParameter.ID = environment.InitializationNumber;
    this.tsyear_inventory_histSearch();
  }
}
