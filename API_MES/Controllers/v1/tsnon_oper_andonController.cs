    ...
    public class tsnon_oper_andonController : BaseController<tsnon_oper_andon, Itsnon_oper_andonService>
    {
        ...
        public virtual async Task<string> C02_STOP_LoadAsync()
        {
            ...
            try
            {
                ...
                result = await _tsnon_oper_andonService.C02_STOP_LoadAsync(baseParameter.Account, baseParameter.SearchString);
            }
            catch (Exception ex)
            {
        ...
        public virtual async Task<string> C02_STOP_Button2_ClickAsync()
        {
            ...
            {
                BaseParameter baseParameter = JsonConvert.DeserializeObject<BaseParameter>(Request.Form["data"]);
                result = await _tsnon_oper_andonService.C02_STOP_Button2_ClickAsync(baseParameter.Account, baseParameter.SearchString);
            }
            catch (Exception ex)
            {
                string message = ex.Message;
        ...
        public virtual async Task<string> C02_STOP_Button1_ClickAsync()
        {
            ...
            {
                BaseParameter baseParameter = JsonConvert.DeserializeObject<BaseParameter>(Request.Form["data"]);
                result = await _tsnon_oper_andonService.C02_STOP_Button1_ClickAsync(baseParameter.Account, baseParameter.SearchString);
            }
            catch (Exception ex)
            {
                string message = ex.Message;
